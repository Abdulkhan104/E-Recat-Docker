...
const accountStatus = [ // Renamed the original array
  { status: 'PENDING_VERIFICATION', title: 'Pending Verification ', description: 'Account Verification Pending' },
  ...
  { status: 'BANNED', title: 'Banned ', description: 'Account is permanantly banned , due to Isshu' },
  { status: 'CLOSED', title: 'close ', description: 'Account is permanantly Closeed' },
];
...
const SellersTable = () => {
  const [selectedStatus, setSelectedStatus] = useState('ACTIVE'); // Renamed the state variable

  const handleChange = (event: any) => {
    setSelectedStatus(event.target.value); // Updated state variable name
  };
    ...
    <div className="pb-5 w-60">
      <FormControl fullWidth>
        ...
        <Select
          ...
          value={accountStatus} // Updated state variable name
          label="Account Status"
          onChange={handleChange}
        >
          {accountStatus.map((item) => ( // Use the renamed array
