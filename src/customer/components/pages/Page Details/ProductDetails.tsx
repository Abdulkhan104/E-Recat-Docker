...
const ProductDetails = () => {
  ...
  const {product} = useAppSellector(store=>store)
    ...
    <div className='px-5 lg:px-20 pt-10'>
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
        <section className='flex flex-col lg:flex-row gap-5'>
          ...
          {/* //side images 4*/}
         <div className='w-full lg:w-[15%] flex-wrap lg:flex-col gap-3'>
           {product.product?.images.map((item ,index)=><img onClick={handleActiveImage(index)}
           className='lg:w-full w-[50px] cursor-pointer rounded-md pt-2'
           src={item}/>)}
         </div>
        ...
        <section>
        <h1 className='font-bold text-primary-color'>{product.product?.seller?.businessDetails.businessName}</h1>
        ...
        <div className='price flex items-center gap-3 mt-5 text-2xl'>
          ...
          <p className='text-sm'>Inclusive of all taxes. Free Shing above ₹1500.
          </p>
          </div>
