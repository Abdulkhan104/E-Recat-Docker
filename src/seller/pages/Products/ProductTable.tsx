...
export default function ProductTable() {
  ...
  // const {sellerProduct} = useAppSellector(store=>store)
  const sellerProducts = useAppSellector((store) => store.sellerProduct.products); // Corrected selector
  ...
  return (
    <TableContainer component={Paper}>
      <Table sx={{ minWidth: 700 }} aria-label="customized table">
        ...
        <TableBody>
          {sellerProducts.map((item:Product) => (
            <StyledTableRow key={item.id}>
              <StyledTableCell component="th" scope="row">
                <div className='flex gap-1 flex-wrap'>
              {item.images.map((image, index)=><img className='w-20 rounded-md' alt='' src={image} key={index}/>)}
                </div>
              </StyledTableCell>
